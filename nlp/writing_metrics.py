...
def _insight_system(group_type: str) -> str:
    ...
    return (
        ...
        "3. COMPARE SEMPRE COM A MÉDIA: um valor só é 'alto', 'baixo', 'elaborado' ou 'enxuto' "
        "quando estiver visivelmente acima ou abaixo da média fornecida. Nunca descreva um valor "
        "como elevado ou baixo sem ancorá-lo explicitamente nessa referência. Diferenças menores "
        f"que 10% da média não merecem destaque.\n"
        "4. CUBRA AS TRÊS MÉTRICAS: a análise deve mencionar Sofisticação, Nominalização e "
        "Comprimento — ainda que brevemente. Nunca foque em apenas uma e ignore as demais.\n"
        f"5. SINTETIZE o quadro geral em vez de listar os {plur} um a um: identifique os {plur} "
        ...
        "6. Foque nos padrões GRANDES E ESTÁVEIS (contrastes nítidos, extremos claros). NÃO "
        "comente diferenças minúsculas nem o ordenamento fino do meio da tabela (são instáveis "
        "e mudam a cada semana).\n"
        "7. NÃO inclua ressalvas sobre o sistema, o período analisado, as fontes dos dados ou "
        "limitações metodológicas — essas informações já estão na interface. Termine no último "
        "achado relevante.\n"
        "8. Seja conciso: 3 a 5 frases, um único parágrafo corrido. Sem listas, sem títulos."
    )
