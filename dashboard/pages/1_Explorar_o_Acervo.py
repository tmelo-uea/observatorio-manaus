...
_texts = [_clean_text(filtered[col]) for col in ["title", "summary"] if col in filtered.columns]
_all_text = " ".join(t for t in _texts if t.strip())
if _all_text.strip():
    _wc = WordCloud(
        width=1600, height=500, background_color="white",
        collocations=True, max_words=120, stopwords=_WORDCLOUD_STOPWORDS,
        regexp=r"\b[^\W\d_]{3,}\b",
        margin=6, max_font_size=120,
    ).generate(_all_text)
    _fig_wc, _ax = plt.subplots(figsize=(16, 5))
    _ax.imshow(_wc, interpolation="bilinear")
    _ax.axis("off")
    _fig_wc.tight_layout(pad=0.5)
    st.pyplot(_fig_wc)
    plt.close(_fig_wc)
