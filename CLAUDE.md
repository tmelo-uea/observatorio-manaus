...
- **`generate_cards.py`** — deterministic carousel generator (no LLM calls at render time): reads `daily_summaries` (general + per-topic), includes a card for every topic with a summary that day — **excluding `seguranca-publica`**, which requires human review before publication (see "Editorial safety" below) — and renders a carousel (1080×1350: cover, one card per eligible topic, closing card) plus a caption, using PIL and the brand palette (navy `#0A2238`, forest green, cream, gold, terracotta). Instagram caps carousels at 10 images, so `_select_cards` drops the closing card first if more than 8 topics have a summary that day, and only trims to the 9 most-covered topics (by article count) if there are still too many (10+) after that. Uses the `DejaVuSans` font bundled with `matplotlib` (already a dependency) instead of shipping a font file.
